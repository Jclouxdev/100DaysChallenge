...
export const PasswordGenerator = () => {
    ...
    useMemo(() => {
        console.log("Test");
        setGeneratedPassword(passwordGeneration(passwordLenght))
    }, [optionLower, optionNumber, optionSymbol, optionUpper])
    
    return (
        <div className="bg-[#F7E7FF] w-full h-full">
            <div className="bg-[#090212] w-full h-full pt-20 text-[#F7E7FF] px-6 flex flex-col gap-8">
                ...
                <div className="grid gap-2">
                    ...
                    <p>{optionUpper ? "true" : "false"}</p>
                    <p>{optionLower ? "true" : "false"}</p>
                    <p>{optionNumber ? "true" : "false"}</p>
                    <p>{optionSymbol ? "true" : "false"}</p>
                </div>
                <button className="bg-[#CE5FFF] rounded-xl px-4 py-3 font-bold">Generate Password</button>
            </div>
        </div>
    )
}